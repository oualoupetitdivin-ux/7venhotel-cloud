...
module.exports = async function authRoutes(fastify) {
  ...
  fastify.post('/rafraichir', async (request, reply) => {
    ...
    try {
      ...
      const nouveauToken = fastify.genererToken(user)
      reply.send({ token: nouveauToken })
    } catch {
  ...
  fastify.post('/changer-mot-de-passe', {
    preHandler: [fastify.authentifier]
  }, async (request, reply) => {
