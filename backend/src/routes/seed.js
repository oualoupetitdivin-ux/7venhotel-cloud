    ...
    try {
      ...
      for (const [id, numero, etage, typeId, statut] of chambres) {
        if (has(colsChambre, 'tenant_id')) {
          ...
          await client.query(`
            INSERT INTO chambres (id, hotel_id, numero, etage, type_chambre_id, statut)
            VALUES ($1,'22222222-2222-2222-2222-222222222222',$2,$3,$4,$5)
            ON CONFLICT (id) DO UPDATE SET statut = $5
          `, [id, numero, etage, typeId, statut])
        }
        ...
        if (has(colsReservations, 'tenant_id')) {
          await client.query(`
            ...
            VALUES ($1,'22222222-2222-2222-2222-222222222222','11111111-1111-1111-1111-111111111111',
              $2,$3,$4,$5,$6,2,$7,'XAF',$8,$8,'direct','44444444-4444-4444-4444-444444444444')
            ON CONFLICT (id) DO NOTHING
          `, [id, clientId, chambreId, statut, arrivee, depart, tarif, total])
        } else {
          await client.query(`
            INSERT INTO reservations (id, hotel_id, client_id, chambre_id, statut,
              date_arrivee, date_depart, nombre_adultes, tarif_nuit, devise,
              total_hebergement, total_general, source, creee_par)
      ...
      for (const [id, nom, desc, categorie, prix] of menu) {
        if (has(colsMenu, 'tenant_id')) {
          await client.query(`
            INSERT INTO articles_menu (id, hotel_id, tenant_id, nom, description, categorie, prix, disponible)
            VALUES ($1,'22222222-2222-2222-2222-222222222222','11111111-1111-1111-1111-111111111111',$2,$3,$4,$5,true)
            ON CONFLICT (id) DO NOTHING
          `, [id, nom, desc, categorie, prix])
        } else {
          await client.query(`
            INSERT INTO articles_menu (id, hotel_id, nom, description, categorie, prix, disponible)
            VALUES ($1,'22222222-2222-2222-2222-222222222222',$2,$3,$4,$5,true)
            ON CONFLICT (id) DO NOTHING
          `, [id, nom, desc, categorie, prix])
        }
      }
      ...
      for (const [id, chambreId, type, priorite, statut, desc] of taches) {
        if (has(colsTaches, 'tenant_id')) {
          ...
          await client.query(`
            INSERT INTO taches_menage (id, hotel_id, chambre_id, type_tache, priorite, statut, description, date_tache)
            VALUES ($1,'22222222-2222-2222-2222-222222222222',$2,$3,$4,$5,$6,CURRENT_DATE)
            ON CONFLICT (id) DO NOTHING
          `, [id, chambreId, type, priorite, statut, desc])
        }
      }
      ...
      for (const [id, chambreId, titre, desc, categorie, priorite, statut] of tickets) {
        if (has(colsTickets, 'tenant_id')) {
          ...
          await client.query(`
            INSERT INTO tickets_maintenance (id, hotel_id, chambre_id, titre, description, categorie, priorite, statut, signale_par)
            VALUES ($1,'22222222-2222-2222-2222-222222222222',$2,$3,$4,$5,$6,$7,'55555555-5555-5555-5555-555555555555')
            ON CONFLICT (id) DO NOTHING
          `, [id, chambreId, titre, desc, categorie, priorite, statut])
        }
      }
      ...
      // ── TAXES ─────────────────────────────────────────────────────
      if (has(colsTaxes, 'tenant_id')) {
        await client.query(`
          INSERT INTO taxes (id, hotel_id, tenant_id, nom, taux, type, applicable_a, actif) VALUES
        ...
        await client.query(`
          INSERT INTO taxes (id, hotel_id, nom, taux, type, applicable_a, actif) VALUES
      ...
      for (let i = 30; i >= 0; i--) {
        ...
        const revResto= Math.floor(rev * 0.15)
        const revTotal= rev + revResto
        const arr     = Math.floor(Math.random() * 5) + 1
        const dep     = Math.floor(Math.random() * 5) + 1
        const chambOcc= Math.floor(occ / 100 * 21)
        if (has(colsAnalytics, 'tenant_id')) {
          await client.query(`
            INSERT INTO analytics_quotidiennes
              (hotel_id, tenant_id, date, taux_occupation, chambres_occupees, chambres_disponibles,
               revenu_hebergement, revenu_restaurant, revenu_total, arrivees, departs)
          ...
          `, [dt, occ, chambOcc, 21, rev, revResto, revTotal, arr, dep])
        } else {
          await client.query(`
            INSERT INTO analytics_quotidiennes
              (hotel_id, date, taux_occupation, chambres_occupees, chambres_disponibles,
               revenu_hebergement, revenu_restaurant, revenu_total, arrivees, departs)
